...
import {
  ...
  ReservedDomains, Design, TokenCollection, TokenX, HOC,
} from './types';
 ...
 */
const asTokenSpec = <C extends DesignableComponents, D extends RequiredDomains>(
  d?: D,
) => (s: Partial <TokenSpec<C, D>>): Partial<TokenSpec<C, D>> => {
    const sKeys = Object.getOwnPropertyNames(s);
    const keys = d
      ? Object.getOwnPropertyNames(d).filter(d => sKeys.includes(d)) 
      : sKeys;
    return pick(s, ...keys);
  };
...
const as = <D extends RequiredDomains = any>(
  ...args$: TokenX<any, D>[]
// @todo figure out a way to type the props of the resulting component better
): HOC<any, any, any> => {
...
): Partial<TokenSpec<C, D>> {
  return mergeWith({}, ...specs, tokenMergeCustomizer);
}
...
/**
     ...
     */
const asElementToken = <D extends RequiredDomains>(d?: D) => asTokenSpec<{}, D>(d);
